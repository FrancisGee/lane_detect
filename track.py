...
def process_img(imge):
	...
	img_eq = cv2.adaptiveThreshold(img_eq,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,7,2)
	#removing any noise which was amplified due to thresholding
	img_eq = cv2.fastNlMeansDenoising(img_eq,None,10,7,21)
	#edge detection
	edg = cv2.Canny(img_eq,2,50)
...
if __name__ == "__main__":
	...
	for fname in imgs:
		...
		top_margin = img.shape[0]*400/768#pixels
		bottom_margin = img.shape[0]*680/768#pixels
		left_lane_left_margin = img.shape[1]*50/1024#pixels
		left_lane_right_margin = img.shape[1]*525/1024#pixels
		right_lane_left_margin = img.shape[1]*525/1024#pixels
		right_lane_right_margin = img.shape[1]*1000/1024#pixels
		height = img.shape[0]

		#load test image
		img_test = cv2.imread(fname,0)
